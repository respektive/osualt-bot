...
import discord
from utils.command_params import (
    COMMAND_FLAGS,
    COG_FLAGS,
...
def get_fun_embed(param):
    ...
    if param == "me":
        ...
        embed.set_thumbnail(
            url="https://cdn.discordapp.com/emojis/931703750983299072.webp?size=96&quality=lossless"
        )
...
class HelpView(discord.ui.View):
    def __init__(self, bot, user):
        ...
        select_options = [
            discord.SelectOption(label=name, value=name.lower()) for name in cog_names
        ]

        # Create a select menu with options for each category
        self.category_select = discord.ui.Select(
            placeholder="Select a category", options=select_options
        )
    ...
    async def on_select(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"{selected_category.capitalize()} Commands", colour=0xCC5288
        )
        embed.description = "\n".join(command_list)
...
class ParametersView(discord.ui.View):
    def __init__(self, bot, user):
        ...
        select_options = [
            discord.SelectOption(
                label=COMMAND_FLAGS[category]["name"], value=category.lower()
            )
    ...
    async def on_select(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=COMMAND_FLAGS[selected_category]["name"], colour=0xCC5288
        )
...
class Info(commands.Cog):
    ...
    async def help(self, ctx, command_name: str = None):
        ...
        command_name = (
            command_name.lower() if command_name else None
        )  # ensure lowercase
        if command_name is None:
            ...
            if cog_flags:
                for flag in cog_flags:
                    embed.add_field(
                        name=COMMAND_FLAGS[flag]["name"],
                        value=COMMAND_FLAGS[flag]["value"],
                        inline=False,
                    )
    ...
    async def info(self, ctx):
        ...
        embed = discord.Embed(
            title=f"Info",
            description="The prefix for this bot is `!`. **Bot is standard only**.\nPermanent Server Invite: https://discord.gg/osualt",
            color=0xCC5288,
        )

        embed.add_field(
            name="Read #info",
            value=" Read <#792857827219079208> to get an idea on how the bot works and how to register.",
            inline=False,
        )
        embed.add_field(
            name="Commands",
            value="Request help for a specific command using `!help command-name` and help for filters can be requested using `!help parameters`.\nFor a full list of every command use `!help`.",
            inline=False,
        )
        embed.add_field(
            name="GitHub",

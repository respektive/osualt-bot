...
class Advanced(commands.Cog):
    ...
    async def getscores(self, ctx, *args):
        ...
        if kwargs.get("-unplayed"):
            await get_beatmap_list(ctx, kwargs, tables, sets)
        else:
            await get_beatmap_list(ctx, kwargs, ["scores"] + tables, sets)
        ...
        if kwargs.get("-unplayed"):
            ...
            if kwargs.get("-o") == "nomodscore":
                ...
                await get_beatmap_list(
                    ...
                    ["scores"],
                    False,
                    ("max(top_score - score) as missing_score"),
                    True,
                )
    ...
    async def query(self, ctx, *args, kwargs=None, title=None):
        ...
        user_id = await get_user_id(ctx, kwargs)
        if user_id is not None and not kwargs.get("-user") and kwargs.get("-nolist"):
            kwargs["-user"] = user_id
        ...
        if kwargs.get("-o"):
            ...
            if kwargs["-o"] == "completion" or kwargs["-o"] == "%":
                ...
                await check_tables(
                    ctx,
                    "round((cast(sum(beatmaps.length) * 100::float / "
                    + beatmap_length
                    + " as numeric)), 3)",
                    "scores",
                    kwargs,
                ...
                await check_tables(
                    ctx, "sum(scores.score)", "scores", kwargs, kwargs["-o"].upper()
                )

            elif kwargs["-o"] == "missingscore":
                await check_tables(
                    ctx,
                    "sum(top_score.top_score - scores.score)",
                    "scores",
                    kwargs,
                    "Missing Score",
                )
                ...
                await check_tables(
                    ctx,
                    "sum(top_score_nomod.top_score_nomod - scores.score)",
                    "scores",
                    kwargs,
                    "Missing Nomod Score",
                )
                ...
                await check_tables(
                    ctx, "MAX(scores.score)", "scores", kwargs, "Scorev0"
                )
                ...
                await check_tables(
                    ...
                    "scores",
                    kwargs,
                    "Lazer Standardised Score",
                )

            elif kwargs["-o"] == "lazerscore_standard_nomod":
                await check_tables(
                    ctx,
                    f"SUM({standardised_nomod})",
                    "scores",
                    kwargs,
                ...
                await check_tables(
                    ctx,
                    "sum(score * (DATE_PART('year', NOW() - approved_date) + DATE_PART('day', NOW() - approved_date) / 365.2425))",
                    "scores",
                    kwargs,
                    "Aged Score",
                )
                ...
                if float(kwargs["-weight"]) > 1 or float(kwargs["-weight"]) < 0:
                    await ctx.reply("NO")
                if kwargs["-o"] == "ppv1":
                    await check_weighted_pp(
                        ctx,
                        f"(weighted_ppv1(array_agg(a.pp ORDER BY a.pp DESC), {kwargs['-weight']}))",
                        kwargs,
                        "Weighted pp",
                    )
                else:
                    await check_weighted_pp(
                        ctx,
                        "(weighted_pp(pp_index, a.pp, "
                        + str(kwargs["-weight"])
                        + ") + bonus_pp(count(pp_index)))",
                        kwargs,
                        "Weighted pp",
                    )
                ...
                await check_weighted_pp(
                    ...
                    "(("
                    + str(kwargs["-xexxar-a"])
                    + " - 1) * weighted_pp(pp_index, a.pp, 0.95) + "
                    + str(kwargs["-xexxar-b"])
                    + " * weighted_pp(pp_index, a.pp, 0.95) * (log(sum(a.pp)) / log(weighted_pp(pp_index, a.pp, 0.95)))) / "
                    + str(kwargs["-xexxar-a"]),
                    kwargs,
                    "Weighted pp",
                )
                ...
                await check_weighted_pp(
                    ...
                    + " * sum(("
                    + str(kwargs["-xexxar-a"])
                    + " + 1) * a.pp / (least(pp_index, 100) + "
                    + str(kwargs["-xexxar-a"])
                    + " + "
                    + str(kwargs["-xexxar-b"])
                    + " * greatest(0, (pp_index - 100)))))",
                    kwargs,
                    "Weighted pp",
                )
                ...
                await check_weighted_pp(
                    ...
                    "sum(a.pp * (1::float + "
                    + str(kwargs["-xexxar-a"])
                    + "::float / pp_index::float) / (pp_index::float + "
                    + str(kwargs["-xexxar-a"])
                    + " / pp_index::float))",
                    kwargs,
                    "Weighted pp",
                ...
                await check_weighted_pp(
                    ctx,
                    "sum(a.accuracy * (1::float + "
                    + str(kwargs["-xexxar-a"])
                    + "::float / pp_index::float) / (pp_index::float + "
                    + str(kwargs["-xexxar-a"])
                    + " / pp_index::float)) / sum((1::float + "
                    + str(kwargs["-xexxar-a"])
                    + "::float / pp_index::float) / (pp_index::float + "
                    + str(kwargs["-xexxar-a"])
                    + " / pp_index::float))",
                ...
                await check_weighted_pp(
                    ctx,
                    "sum(a.pp * greatest(1.2 * POW(0.99, pp_index) * (5.1 - pp_index::float) / (ABS(5.1 - pp_index::float)), (1::float + "
                    + str(kwargs["-xexxar-a"])
                    + "::float / pp_index::float) / (pp_index::float + "
                    + str(kwargs["-xexxar-a"])
                    + " / pp_index::float)))",
                ...
                await check_weighted_pp(
                    ...
                    + "::float / pp_index::float) / (pp_index::float + "
                    + str(kwargs["-xexxar-a"])
                    + " / pp_index::float)) - (weighted_pp(pp_index, a.pp, 0.95) + bonus_pp(count(pp_index))))",
                    kwargs,
                    "Weighted pp",
                )
                ...
                if kwargs.get("-registered"):
                    if kwargs["-registered"] == "false" and not kwargs.get("-user"):
                        await ctx.reply("NO")
                        return
                await check_weighted_score(
                    ctx,
                    "weighted_pp(score_index, a.score, 0.95)",
                    kwargs,
                    "Weighted Score",
                )
                ...
                await check_tables(
                    ctx,
                    "count( distinct beatmaps.set_id )",
                    "scores",
                    kwargs,
                    "Mapset Clears",
                )
            ...
            await check_tables(
                ctx, "count( scores.beatmap_id )", "scores", kwargs, title or "Clears"
            )
    ...
    async def averagestars(self, ctx, *args):
        ...
        await ctx.invoke(
            self.bot.get_command("query"),
            kwargs=kwargs,
            title="Average star rating of beatmaps",
        )
    ...
    async def averagelength(self, ctx, *args):
        ...
        await ctx.invoke(
            self.bot.get_command("query"),
            kwargs=kwargs,
            title="Average length of beatmaps",
        )
    ...
    async def averageacc(self, ctx, *args):
        ...
        await ctx.invoke(
            self.bot.get_command("query"), kwargs=kwargs, title="Average acc"
        )
    ...
    async def topstars(self, ctx, *args):
        ...
        await ctx.invoke(
            self.bot.get_command("query"), kwargs=kwargs, title="Top Star Rating"
        )
    ...
    @commands.command(
        aliases=["fcrate", "fcefficiency", "fc_efficiency", "fc_ratio", "fcratio"]
    )
    ...
    @commands.command(
        aliases=["ssrate", "ssefficiency", "ss_efficiency", "ss_ratio", "ssratio"]
    )
    ...
    @commands.command(
        aliases=["srate", "sefficiency", "s_efficiency", "s_ratio", "sratio"]
    )
    ...
    @commands.command(
        aliases=["arate", "aefficiency", "a_efficiency", "a_ratio", "aratio"]
    )
    ...
    async def unique_dt_fc(self, ctx, *args):
        ...
        await check_tables(
            ctx, "count(*)", "unique_dt_fc", kwargs, "Unique DT FC Count"
        )
...
Maximum amount of scores a user is allowed to queue is 1000. To check the queue time, use `!queuelength`."""
        ...
        if kwargs.get("-unplayed"):
            ...
            rows = await get_beatmap_ids(
                kwargs, ["scores", "fc_count", "ss_count", "mods"]
            )

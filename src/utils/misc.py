...
def OADoubleNow():
    return (datetime.datetime.now(datetime.timezone.utc) - OA_EPOC).total_seconds() / OA_S_PER_DAY
...
async def generateosdb(ctx, di):
    ...
    if not di.get("-unplayed"):
        if di.get("-u") or di.get("-missingscore"):
            query = query + " inner join scores on scores.beatmap_id = beatmaps.beatmap_id"
            count = count + " inner join scores on scores.beatmap_id = beatmaps.beatmap_id"
            if not di.get("-registered"):
                di["-registered"] = "true"

    if di.get("-o"):
        if di["-o"] == "neverbeenssed":
            query = query + " inner join neverbeenssed on neverbeenssed.beatmap_id = beatmaps.beatmap_id"
            count = count + " inner join neverbeenssed on neverbeenssed.beatmap_id = beatmaps.beatmap_id"
        if di["-o"] == "neverbeenfced":
            query = query + " inner join neverbeenfced on neverbeenfced.beatmap_id = beatmaps.beatmap_id"
            count = count + " inner join neverbeenfced on neverbeenfced.beatmap_id = beatmaps.beatmap_id"
    ...
    if di.get("-o") and di["-o"] == "score" or (di.get("-scorepersecond") or di.get("-scorepersecond-min") or di.get("-scorepersecond-max") or di.get("-missingscore")):
    ...
    if len(li) > 0:
        for entry in li:
            ...
            if(differential >= 7):
                ...
                embed = discord.Embed(title = 'A map has been FCed with DT for the first time after ' + str(differential) + ' days!', colour=discord.Colour(0xE5E242))
                author = {
                    "name": f"""{username + " - " + str(pp) + "pp #" + str(rank) if result is not None else str(username)}""",
                    "url": f"https://osu.ppy.sh/users/{user}",
                    "icon_url": f"https://a.ppy.sh/{user}"
